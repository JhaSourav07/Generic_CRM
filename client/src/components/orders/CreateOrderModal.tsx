...
export const CreateOrderModal: React.FC<CreateOrderModalProps> = ({
  ...
  const removeItem = (index: number) => {
    if (items.length <= 1) {
      toast({ type: 'warning', title: 'Cannot Remove', message: 'An order must contain at least one line item.' });
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        type: 'success',
        title: 'Order Created',
        message: 'Commercial order successfully recorded.'
      });
      ...
      toast({
        ...
        message: err.message || 'Could not save order.'
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Dialog isOpen={isOpen} onClose={onClose} title="Create Manual Commercial Order" maxWidth="lg">
      <form onSubmit={handleSubmit} className="space-y-4 pt-2">
              ...
              >
                <div className="grid grid-cols-12 gap-2 items-center">
                  <div className="col-span-12 md:col-span-4">
                    <label className="block text-[10px] text-vynexa-text-muted mb-0.5">Product</label>
                    <Select
                      value={it.productId || ''}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-vynexa-border">
          <div>
            ...
            <Textarea
              rows={3}
              placeholder="Commercial terms, PO number, fulfillment notes..."
              value={notes}
          ...
          <div className="bg-vynexa-surface-secondary/60 p-3 rounded-md border border-vynexa-border/60 flex flex-col justify-between">
            ...
            <span className="text-[10px] text-vynexa-text-muted mt-2 block italic">
              * Authoritative calculations are enforced server-side.
            </span>
          </div>
        </div>
