...
export const CreateQuoteModal: React.FC<CreateQuoteModalProps> = ({
  ...
  const [opportunityId, setOpportunityId] = useState<string>(initialOpportunityId || '');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (initialOpportunityId) setOpportunityId(initialOpportunityId);

      // Default valid until 30 days ahead
      const nextMonth = new Date();
      nextMonth.setDate(nextMonth.getDate() + 30);
      setValidUntil(nextMonth.toISOString().split('T')[0]);
    }
  }, [isOpen, initialAccountId, initialOpportunityId]);
  ...
  const removeItem = (index: number) => {
    if (items.length <= 1) {
      toast({ type: 'warning', title: 'Cannot Remove', message: 'A quote must contain at least one line item.' });
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (!it.description.trim()) {
        toast({ type: 'error', title: 'Missing Information', message: `Line item #${i + 1} must have a description.` });
        return;
      }
      if (Number(it.quantity) < 1) {
        toast({ type: 'error', title: 'Invalid Quantity', message: `Line item #${i + 1} quantity must be at least 1.` });
        return;
      }
      if (Number(it.unitPrice) < 0) {
        toast({ type: 'error', title: 'Invalid Price', message: `Line item #${i + 1} unit price cannot be negative.` });
        return;
      }
    }
    ...
    try {
      ...
      toast({
        type: 'success',
        title: 'Quote Created',
        message: 'Draft quote created with server-verified calculations.'
      });
      ...
      toast({
        ...
        message: err.message || 'Could not save quote.'
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Dialog isOpen={isOpen} onClose={onClose} title="Create Commercial Proposal (Quote)" maxWidth="lg">
      <form onSubmit={handleSubmit} className="space-y-4 pt-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-vynexa-border">
          <div>
            ...
            <Textarea
              rows={3}
              placeholder="Commercial terms, payment milestones, net-30 provisions..."
              value={notes}
          ...
          <div className="bg-vynexa-surface-secondary/60 p-3 rounded-md border border-vynexa-border/60 flex flex-col justify-between">
            ...
            <span className="text-[10px] text-vynexa-text-muted mt-2 block italic">
              * Official totals are strictly calculated and persisted server-side.
            </span>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-vynexa-border">
          ...
          <Button type="submit" variant="primary" disabled={loading}>
            {loading ? 'Creating Quote...' : 'Save Draft Quote'}
          </Button>
        </div>
      </form>
    </Dialog>
